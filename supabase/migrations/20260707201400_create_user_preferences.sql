-- AmplifyHub — user_preferences table
-- Backs every field on the Settings page (Account, Appearance, AI Coach,
-- Learning, Language & Region, Privacy). One row per user.
--
-- Originally shipped as assets/user_preferences.sql in the website repo,
-- run by hand via the Supabase SQL Editor. Reproduced here as a tracked
-- migration so `supabase db push`/`db diff` can verify it's actually live.
