...
async function consumeQuota(supabaseClient: ReturnType<typeof createClient>, bucket: 'chat' | 'feedback') {
  const { data, error } = await supabaseClient.rpc('consume_coach_quota', { p_bucket: bucket })
  if (error) throw new Error('Quota service unavailable')
  const result = Array.isArray(data) ? data[0] : data
  if (!result || result.allowed !== true) {
    const retryAfter = Math.max(1, Number(result?.retry_after) || 60)
    throw new HttpError(429, 'RATE_LIMITED', 'AI Coach usage limit reached.', retryAfter)
  }
  return result
}
...
serve(async (req: Request) => {
  ...
  try {
    ...
    const { messages, mode, prefs, action } = validateRequest(parsedBody, {
    ...
    console.log(`coach-chat: action=${action} mode=${mode || 'feedback'} msgCount=${messages.length}`)

    if (!apiKey) {
      throw new Error('Missing Gemini API Key')
    }

    await consumeQuota(supabaseClient, action)
